...
(*   let hashtable = Hashtbl.create (List.length anomaly_list) in *)
...
(* let apply_anomaly_container_five_tuple_flow_detailed_metrics_container_parallelized_list_old *)
...
(*     = *)
(*   ( *)
(*     let five_tuple_flow_detailed_metrics_tuple_list = *)
(*       Five_tuple_flow_detailed_metrics_container.to_list *)
(*         five_tuple_flow_detailed_metrics_container *)
(*     in *)

(*     let detailed_metrics_int_map_to_use = t.detailed_metrics_int_map in *)
...
(*       Map_data.fold_list *)
(*         parallelization_mode *)
(*         (fun hashtable_1 (five_tuple_flow, five_tuple_flow_detailed_metrics) -> *)
(*           ( *)
(*             let src_ip_option *)
(*           , dst_ip_option *)
(*             , proto_option *)
...
(*           , dst_port_option *)
(*                         = *)
(*               Five_tuple_flow.to_tuple_option *)
(*                 five_tuple_flow *)
(*             in *)

(*             (\* print_endline "Anomaly_detailed_metrics_container: apply: checking timestamp"; *\) *)

(*             (\* let timestamp_sec_start = five_tuple_flow_detailed_metrics.Five_tuple_flow_detailed_metrics.timestamp_sec_start in *\) *)
(*             (\* let timestamp_usec_start = five_tuple_flow_detailed_metrics.Five_tuple_flow_detailed_metrics.timestamp_usec_start in *\) *)
(*             (\* let timestamp_sec_end = five_tuple_flow_detailed_metrics.Five_tuple_flow_detailed_metrics.timestamp_sec_end in *\) *)
(*             (\* let timestamp_usec_end = five_tuple_flow_detailed_metrics.Five_tuple_flow_detailed_metrics.timestamp_usec_end in *\) *)

(*             (\* if timestamp_sec_start >= timestamp_sec_end then *\) *)
(*             (\*   ( *\) *)
(*             (\*     if timestamp_usec_start > timestamp_usec_end then *\) *)
(*             (\*       ( *\) *)
(*             (\*         print_endline *\) *)
(*             (\*           (sprintf *\) *)
(*             (\*              "Anomaly_detailed_metrics_container: apply: flow start is not before flow end: %d.%d >= %d.%d" *\) *)
(*             (\*              timestamp_sec_start *\) *)
...
(*             (\*         print_endline *\) *)
(*             (\*           (sprintf *\) *)
(*             (\*              "Anomaly_detailed_metrics_container: apply:\n%s\n%s\n%s\n%d\n%d" *\) *)
(*             (\*              (match src_ip_option with | None -> "" | Some addr -> Uri_IP.ipv4_to_string addr) *\) *)
(*             (\*              (match dst_ip_option with | None -> "" | Some addr -> Uri_IP.ipv4_to_string addr) *\) *)
(*             (\*              (match proto_option with | None -> "" | Some proto -> proto) *\) *)
(*             (\*              (match src_port_option with | None -> 0 | Some port -> port) *\) *)
(*             (\*              (match dst_port_option with | None -> 0 | Some port -> port) *\) *)
(*             (\*           ); *\) *)
(*             (\*         print_endline *\) *)
(*             (\*           (sprintf *\) *)
(*             (\*              "Anomaly_detailed_metrics_container: apply:\n%s\n\n%s\n" *\) *)
...
(*             (\*                 five_tuple_flow) *\) *)
(*             (\*              (Five_tuple_flow_detailed_metrics.to_string *\) *)
(*             (\*                 To_string_mode.Simple *\) *)
...
(*             (\* assert(timestamp_sec_start < timestamp_sec_end); *\) *)
...
(*               then *)
(*                 ( *)
(*                   (\* debug "add_packet: anomaly %d updating" anomaly.Admd.Anomaly.indice; *\) *)
...
(*       (\*   Detailed_metrics.of_five_tuple_flow_detailed_metrics *\) *)
(*       (\*     five_tuple_flow *\) *)
(*       (\*     five_tuple_flow_detailed_metrics *\) *)
(*       (\* in *\) *)
...
(*                         (\* print_endline *\) *)
(*                         (\*   (sprintf *\) *)
(*                         (\*      "Anomaly_detailed_metrics_container: apply: problem with:\n%s\n\n%s\n" *\) *)
(*                         (\*      (Five_tuple_flow.to_string *\) *)
(*                         (\*         five_tuple_flow) *\) *)
(*                         (\*      (Five_tuple_flow_detailed_metrics.to_string *\) *)
(*                         (\*         To_string_mode.Simple *\) *)
...
(*           ) *)
(*         ) *)
(*         (fun (hashtable_1 : (int, Detailed_metrics.t) Hashtbl.t) hashtable_2 -> *)
...
(*           new_hashtable *)
(*         ) *)
(*         detailed_metrics_int_map_to_use *)
(*         (\* (Hashtbl.create nb_anomaly) *\) *)
(*         five_tuple_flow_detailed_metrics_tuple_list *)
(*     in *)
